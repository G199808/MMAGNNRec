...
def load_amazon_dataset(data_path):
    ...
    with open(test_path, 'r') as f:
        for line in f:
            ...
            u = parts[0]
            items = parts[1:]
            test_user_set[u] = set(items)
            num_users = max(num_users, u + 1)
            for i in items:
                num_items = max(num_items, i + 1)

    # 3. 读取预提取的高维模态特征 [cite: 121, 122]
...
class BPRDataset(Dataset):
    ...
    严格确保采样的负样本 item 绝不在该用户的历史训练交互集合中 [cite: 107]
    """
...
def calculate_metrics(topk_items, test_ground_truth, k_list=[10, 20]):
    """
    精确计算推荐系统的核心全量评测指标 Recall@K 和 NDCG@K [cite: 130, 131]
    """
